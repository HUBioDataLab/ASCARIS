...
opts = detectImportOptions('Paper_files/originaldata/training_uptodate_full_2014selected_psnp.txt');
...
T_benchmark_ps = readtable('Paper_files/originaldata/training_uptodate_full_2014selected_psnp.txt',opts);
neut_delet_ps = table2array(T_benchmark_ps(:,78));
T_full_ps = T_benchmark_ps(:,[9:12 14:77]);

T_benchmark_ps_test = readtable('Paper_files/benchmark/psnp.txt',opts);
neut_delet_ps_test = table2array(T_benchmark_ps_test(:,78));
T_full_ps_test = T_benchmark_ps_test(:,[9:12 14:77]);

save PredictSNP_benchmark_modeling/T_all_ps.mat T_full_ps neut_delet_ps T_full_ps_test neut_delet_ps_test T_benchmark_ps_test T_benchmark_ps



% Training and testing:

Mdl_full_cval_ps=fitcensemble(T_full_ps,neut_delet_ps,'Method','Bag','CrossVal','on','KFold',5);
...
Mdl_full_test_ps=fitcensemble(T_full_ps,neut_delet_ps,'Method','Bag');
...
Perf_table_ps_test_family=table(Family,AUC,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);

family_ps_test=table2array(T_benchmark_ps_test(:,8));
ind_enzyme=find(contains(family_ps_test,'Enzymes')==1);neut_delet_ps_test_fam{1,1}=neut_delet_ps_test(ind_enzyme,1);testPred_fam{1,1}=testPredictions(ind_enzyme,1);testScor_fam{1,1}=testScores(ind_enzyme,:);
ind_membrane=find(contains(family_ps_test,'Membrane')==1);neut_delet_ps_test_fam{2,1}=neut_delet_ps_test(ind_membrane,1);testPred_fam{2,1}=testPredictions(ind_membrane,1);testScor_fam{2,1}=testScores(ind_membrane,:);
ind_tfactor=find(contains(family_ps_test,'Transcription')==1);neut_delet_ps_test_fam{3,1}=neut_delet_ps_test(ind_tfactor,1);testPred_fam{3,1}=testPredictions(ind_tfactor,1);testScor_fam{3,1}=testScores(ind_tfactor,:);
ind_ion=find(contains(family_ps_test,'Ion')==1);neut_delet_ps_test_fam{4,1}=neut_delet_ps_test(ind_ion,1);testPred_fam{4,1}=testPredictions(ind_ion,1);testScor_fam{4,1}=testScores(ind_ion,:);
ind_epigen=find(contains(family_ps_test,'Epigenetic')==1);neut_delet_ps_test_fam{5,1}=neut_delet_ps_test(ind_epigen,1);testPred_fam{5,1}=testPredictions(ind_epigen,1);testScor_fam{5,1}=testScores(ind_epigen,:);
ind_others=find(contains(family_ps_test,'nan')==1);neut_delet_ps_test_fam{6,1}=neut_delet_ps_test(ind_others,1);testPred_fam{6,1}=testPredictions(ind_others,1);testScor_fam{6,1}=testScores(ind_others,:);
ind_others4=unique([ind_tfactor;ind_ion;ind_epigen;ind_others]);neut_delet_ps_test_fam{7,1}=neut_delet_ps_test(ind_others4,1);testPred_fam{7,1}=testPredictions(ind_others4,1);testScor_fam{7,1}=testScores(ind_others4,:);
neut_delet_ps_test_fam{8,1}=neut_delet_ps_test;testPred_fam{8,1}=testPredictions;testScor_fam{8,1}=testScores;

for i=1:8
    confmat=confusionmat(neut_delet_ps_test_fam{i,1},testPred_fam{i,1});confmat_fix=[confmat(2,2) confmat(2,1);confmat(1,2) confmat(1,1)];[~,~,~,AUC]=perfcurve(neut_delet_ps_test_fam{i,1},testScor_fam{i,1}(:,2),1);TP=confmat_fix(1,1);TN=confmat_fix(2,2);FN=confmat_fix(1,2);FP=confmat_fix(2,1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));Perf_table_temp=table(AUC,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
    Perf_table_ps_test_family(i,2:11)=Perf_table_temp;
end

save PredictSNP_benchmark_modeling/Perf_tables_ps_family.mat Perf_table_ps_test_family
...
ps_input_data = readtable('Paper_files/benchmark/inputs_to_create_featurevector_benchmark/psnp_input.txt');
c_in=[table2array(ps_input_data(:,1:2)) num2cell(table2array(ps_input_data(:,3))) table2array(ps_input_data(:,4))];
c_in=cellfun(@string,c_in);
c_in={join(c_in,'')};d_in=cellstr(c_in{1,1});

ps_perf_other_methods = readtable('Paper_files/benchmark/predictions_other_methods/psnp_preds.txt');
...
[Lia,Locb]=ismember(T_benchmark_ps_test.meta_merged,d_in);
length(find(Lia==1))
[Lia,Locb]=ismember(T_benchmark_ps_test.meta_merged,d);
length(find(Lia==1))
ps_perf_other_methods_ourdataset=ps_perf_other_methods(Locb,:);
...
ps_perf_Logit_=ps_perf_other_methods.Logit_(Locb);TP=length(find(ismember(ps_perf_Logit_,'TP'))==1);TN=length(find(ismember(ps_perf_Logit_,'TN'))==1);FN=length(find(ismember(ps_perf_Logit_,'FN'))==1);FP=length(find(ismember(ps_perf_Logit_,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(1,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
ps_perf_Logit__=ps_perf_other_methods.Logit__(Locb);TP=length(find(ismember(ps_perf_Logit__,'TP'))==1);TN=length(find(ismember(ps_perf_Logit__,'TN'))==1);FN=length(find(ismember(ps_perf_Logit__,'FN'))==1);FP=length(find(ismember(ps_perf_Logit__,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(2,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
ps_perf_Condel_=ps_perf_other_methods.Condel_(Locb);TP=length(find(ismember(ps_perf_Condel_,'TP'))==1);TN=length(find(ismember(ps_perf_Condel_,'TN'))==1);FN=length(find(ismember(ps_perf_Condel_,'FN'))==1);FP=length(find(ismember(ps_perf_Condel_,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(3,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
ps_perf_Condel__=ps_perf_other_methods.Condel__(Locb);TP=length(find(ismember(ps_perf_Condel__,'TP'))==1);TN=length(find(ismember(ps_perf_Condel__,'TN'))==1);FN=length(find(ismember(ps_perf_Condel__,'FN'))==1);FP=length(find(ismember(ps_perf_Condel__,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(4,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
ps_perf_FatHMM_W_=ps_perf_other_methods.FatHMM_W_(Locb);TP=length(find(ismember(ps_perf_FatHMM_W_,'TP'))==1);TN=length(find(ismember(ps_perf_FatHMM_W_,'TN'))==1);FN=length(find(ismember(ps_perf_FatHMM_W_,'FN'))==1);FP=length(find(ismember(ps_perf_FatHMM_W_,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(5,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
ps_perf_FatHMM_U_=ps_perf_other_methods.FatHMM_U_(Locb);TP=length(find(ismember(ps_perf_FatHMM_U_,'TP'))==1);TN=length(find(ismember(ps_perf_FatHMM_U_,'TN'))==1);FN=length(find(ismember(ps_perf_FatHMM_U_,'FN'))==1);FP=length(find(ismember(ps_perf_FatHMM_U_,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(6,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
ps_perf_LRT_=ps_perf_other_methods.LRT_(Locb);TP=length(find(ismember(ps_perf_LRT_,'TP'))==1);TN=length(find(ismember(ps_perf_LRT_,'TN'))==1);FN=length(find(ismember(ps_perf_LRT_,'FN'))==1);FP=length(find(ismember(ps_perf_LRT_,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(7,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
ps_perf_SIFT_=ps_perf_other_methods.SIFT_(Locb);TP=length(find(ismember(ps_perf_SIFT_,'TP'))==1);TN=length(find(ismember(ps_perf_SIFT_,'TN'))==1);FN=length(find(ismember(ps_perf_SIFT_,'FN'))==1);FP=length(find(ismember(ps_perf_SIFT_,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(8,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
ps_perf_MutationTaster_=ps_perf_other_methods.MutationTaster_(Locb);TP=length(find(ismember(ps_perf_MutationTaster_,'TP'))==1);TN=length(find(ismember(ps_perf_MutationTaster_,'TN'))==1);FN=length(find(ismember(ps_perf_MutationTaster_,'FN'))==1);FP=length(find(ismember(ps_perf_MutationTaster_,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(9,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
ps_perf_MutationAssessor_=ps_perf_other_methods.MutationAssessor_(Locb);TP=length(find(ismember(ps_perf_MutationAssessor_,'TP'))==1);TN=length(find(ismember(ps_perf_MutationAssessor_,'TN'))==1);FN=length(find(ismember(ps_perf_MutationAssessor_,'FN'))==1);FP=length(find(ismember(ps_perf_MutationAssessor_,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(10,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
ps_perf_PolyPhen2_=ps_perf_other_methods.PolyPhen2_(Locb);TP=length(find(ismember(ps_perf_PolyPhen2_,'TP'))==1);TN=length(find(ismember(ps_perf_PolyPhen2_,'TN'))==1);FN=length(find(ismember(ps_perf_PolyPhen2_,'FN'))==1);FP=length(find(ismember(ps_perf_PolyPhen2_,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_ps_other_methods_overall(11,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
