...
save MT_benchmark_modeling/T_all_mt.mat T_full_mt neut_delet_mt T_full_mt_test neut_delet_mt_test T_full_2019train_mt neut_delet_2019train_mt T_benchmark_mt_test T_benchmark_2019train_mt T_benchmark_mt
...
Perf_table_2019train_mt_test=table(AUC,NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
...
save MT_benchmark_modeling/Mdl_full_mt.mat Mdl_full_cval_mt Mdl_full_test_mt Mdl_full_2019train_test_mt
save MT_benchmark_modeling/Perf_tables_mt.mat Perf_table_mt_cval Perf_table_mt_test Perf_table_2019train_mt_test
...
maxNumSplits = 3.^(0:m);
maxNumSplits = [maxNumSplits([8]) size(T_full_mt,1)-1];
...
save MT_benchmark_modeling/Mdl_mt_hypopt.mat Mdl_rf_mt_hypopt Mdl_rf_2019train_mt_hypopt
save MT_benchmark_modeling/Perf_tables_mt_hyperpar.mat Perf_table_mt_test_hyperpar Perf_table_2019train_mt_test_hyperpar
...
Family=({'Enzymes';'Membrane_receptors';'Transcription_factors';'Ion_channels';'Epigenetic_regulators';'Others';'Others_4';'Overall'});
NPV=zeros(8,1);Specificity=zeros(8,1);AUC=zeros(8,1);Recall=zeros(8,1);Precision=zeros(8,1);F1score=zeros(8,1);Accuracy=zeros(8,1);MCC=zeros(8,1);TP=zeros(8,1);FN=zeros(8,1);FP=zeros(8,1);TN=zeros(8,1);
Perf_table_2019train_mt_test_family=table(Family,NPV,Specificity,AUC,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
...
save MT_benchmark_modeling/Perf_tables_mt_family.mat Perf_table_mt_test_family Perf_table_2019train_mt_test_family
...
mt_perf_PPH2div=mt_perf_other_methods.PPH2_div(Locb);TP=length(find(ismember(mt_perf_PPH2div,'TP'))==1);TN=length(find(ismember(mt_perf_PPH2div,'TN'))==1);FN=length(find(ismember(mt_perf_PPH2div,'FN'))==1);FP=length(find(ismember(mt_perf_PPH2div,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_mt_other_methods_overall(1,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
...
mt_perf_MT=mt_perf_other_methods.MT(Locb);TP=length(find(ismember(mt_perf_MT,'TP'))==1);TN=length(find(ismember(mt_perf_MT,'TN'))==1);FN=length(find(ismember(mt_perf_MT,'FN'))==1);FP=length(find(ismember(mt_perf_MT,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_mt_other_methods_overall(3,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
...
mt_perf_PROVEAN=mt_perf_other_methods.PROVEAN(Locb);TP=length(find(ismember(mt_perf_PROVEAN,'TP'))==1);TN=length(find(ismember(mt_perf_PROVEAN,'TN'))==1);FN=length(find(ismember(mt_perf_PROVEAN,'FN'))==1);FP=length(find(ismember(mt_perf_PROVEAN,'FP'))==1);Recall=TP/(TP+FN);Precision=TP/(TP+FP);F1score=(2*TP)/(2*TP+FP+FN);Accuracy=(TP+TN)/(TP+FP+FN+TN);MCC=((TP*TN)-(FP*FN))/(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))^(1/2));NPV=TN/(TN+FN);Specificity=TN/(TN+FP);Perf_table_mt_other_methods_overall(5,2:12)=table(NPV,Specificity,Recall,Precision,F1score,Accuracy,MCC,TP,FN,FP,TN);
